...
function resolveRepoUrl(spec, env) {
  const explicitUrl =
    typeof env?.[spec.repoUrlEnvVar] === 'string' ? env[spec.repoUrlEnvVar].trim() : '';
...
function resolveTargetRef({ spec, repoUrl, env, checkoutRoot, syncExistingRepos }) {
  const explicitRepoRef =
    typeof env?.[spec.refEnvVar] === 'string' ? env[spec.refEnvVar].trim() : '';
  if (explicitRepoRef.length > 0) {
    return explicitRepoRef;
  }

  const explicitRef =
    typeof env?.[SHARED_SDK_GIT_REF_ENV_VAR] === 'string'
...
function cloneSourceRepo({ repoRoot, repoUrl, targetRef }) {
  ...
  run('git', ['clone', '--no-checkout', repoUrl, repoRoot]);
...
function syncExistingSourceRepo({ repoRoot, repoUrl, targetRef, label }) {
  ...
  run('git', ['-C', repoRoot, 'fetch', 'origin', '--tags', '--prune']);
  run('git', ['-C', repoRoot, 'checkout', '--force', targetRef]);
...
export function ensureSharedSdkGitSources({
  ...
  env = process.env,
  syncExistingRepos =
    parseBooleanFlag(env?.CI) || parseBooleanFlag(env?.[SHARED_SDK_GIT_FORCE_SYNC_ENV_VAR]),
} = {}) {
